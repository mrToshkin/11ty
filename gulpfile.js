...
const watcher = () => {
  ...
  // watch('./src/**/*.scss', series(scssDev)).on('change', browserSync.reload);
  watch('./src/**/*.scss', series(scssDev));
  watch('./src/scripts/*.js', series(js));
  // watch('./src/**/*.pug', series(pug)).on('change', browserSync.reload);
  // watch('./src/fonts/**', series(fonts));
};

// exports.optimg = optimg = series(cleanImg, img, webp, svg, sprite, spriteSocial);
const dev = series(cleanBuild, parallel(scssDev, js));
